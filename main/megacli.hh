...
namespace CLI
{
    ...
    constexpr const char *PROMPT_ROOT{"ROOT> "};
    constexpr const char *PROMPT_USER{"USER> "};
...
cb->printf("Welcome to MegaCLI v0.0.1. Type 'help' to get help.\r\n");
        ...
        int insertChar(char c, ShellId shellId, ShellCallback *cb)
        {
            ...
            ESP_LOGI(TAG, "linenoiseEditInsert old_history_index=%i historyIndex=%i, char=%c, pos=%u, strlen=%u", old_history_index, history_index, c, pos, history.front().size());
        ...
        {
            ESP_LOGI(TAG, "moveInHistory history_index=%d, history_size=%u, end=%s", history_index, history.size(), history.back().c_str());
            if (directionNext)
            {
        ...
        void doBackspace(ShellId shellId, ShellCallback *cb)
        {
            ...
            ESP_LOGI(TAG, "doBackspace pos=%u len=%u", pos, len);
            if (pos > 0 && len > 0)
            {
     ...
     void copyCurrentHistoryToFrontForEdit(ShellId shellId, ShellCallback *cb)
        {
            if (history_index == 0)
                return;
            ESP_LOGI(TAG, "copyCurrentHistoryToFrontForEdit");
            history.front().clear();
        ...
        int executeLine(ShellId shellId, ShellCallback *cb)
        {
            ...
            int argc = 0;
            char buf[200];
            strcpy(buf, history.front().c_str());
            ESP_LOGI(TAG, "executeLine with buf=%s and string len=%u", buf, history.front().length());
            history.AddToQueueConsiderSizeLimit("");
            history.front().reserve(MAX_LINE_LENGTH);
            pos=0;
            //history.front().shrink_to_fit();
            ...
            for (auto &cmd : this->commands)
            {
                ...
                {
                    int retval=cmd->Execute(cb, argc, argv);
                    if(retval!=0){
                        cb->printf(COLOR_RESET COLOR_YELLOW "\r\nCommand %s returned %d\r\n" COLOR_RESET, buf, retval);
                    }
                    refreshLine(cb);
                    return 0;
                }
            }
            cb->printf(COLOR_BRIGHT_RED "Unknown Command: %s\r\n" COLOR_RESET, buf);
            refreshLine(cb);
            return -1;
        }
        ...
        int handleChars(const char *chars, size_t len, ShellId shellId, ShellCallback *cb)
        {
            ...
            { // we received an escape sequence
              // we assume: the whole escape sequence comes in one call
                ...
                /*
                    if (chars[1] == '[' && chars[len] == 'R')
                    {
                        char *posOfSemicolon{nullptr};
                        int rows = strtol(&chars[2], &posOfSemicolon, 10);
                        if (*posOfSemicolon != ';')
                            return -1;
                        this->cols = strtol(posOfSemicolon + 1, nullptr, 10);
                        ESP_LOGW(TAG, "Received an escape sequence for rows=%d and cols=%d", rows, cols);
                        // xEventGroupSetBits(this->genericEventGroup, EVBIT_CURSOR_POS_RECEIVED);
                        return 0;
                    }
                    return -1;
            ...
            // wenn es keine EscapeSequence ist, müssen die Zeichen einzeln kommen!
            if (len > 1)
            {
                ESP_LOGW(TAG, "Mehr als ein Zeichen!!");
            }
            return handleSingleChar(chars[0], shellId, cb);
        }
