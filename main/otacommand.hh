...
{
private:
	int ExecuteInternal(ShellCallback *cb, const char *url, bool showInfoOnly, bool force)
	{
		int exitcode{0};
		...
		{
			cb->printf(COLOR_RESET COLOR_RED "At least one partition, that is needed for OTA operation, ist not available. Check partition table!\r\n" COLOR_RESET);
		...
		{
			cb->printf(COLOR_RESET COLOR_RED "Configured OTA boot partition at offset 0x%08x, but running from offset 0x%08x. (This can happen if either the OTA boot data or preferred boot image become corrupted somehow).\r\n" COLOR_RESET, (unsigned int)configured_partition->address, (unsigned int)running_partition->address);
		...
		{
			cb->printf(COLOR_RESET COLOR_YELLOW "Last invalid firmware version: %s\r\n" COLOR_RESET, app_desc_invalid.version);
		...
		if (netconn_gethostbyname_addrtype("microsoft.com", &addr, NETCONN_DNS_IPV4) != ERR_OK)
		{
			cb->printf(COLOR_RESET COLOR_RED "Update Process shound start, but there is no internet connectivity\r\n" COLOR_RESET);
			ESP_LOGE(TAG, "Update Process shound start, but there is no internet connectivity");
		}
		...
		{
			cb->printf(COLOR_RESET COLOR_RED "ESP HTTPS OTA Begin failed\r\n" COLOR_RESET);
		...
		if (esp_https_ota_get_img_desc(https_ota_handle, &app_desc_ota) != ESP_OK)
		{
			...
			cb->printf(COLOR_RESET COLOR_RED "esp_https_ota_get_img_desc failed\r\n" COLOR_RESET);
		...
		{
			cb->printf(COLOR_RESET COLOR_YELLOW "New version is the same as invalid version.\r\n" COLOR_RESET);
		...
		{
			cb->printf(COLOR_RESET COLOR_YELLOW "Different project name! remote=\"%s\", local=\"%s\"\r\n" COLOR_RESET, app_desc_ota.project_name, app_desc_run.project_name);
		...
		cb->printf(COLOR_RESET COLOR_GREEN "There is a new version %s available for firmware \"%s\" compiled on %s %s. We try to write this to partition subtype %d at offset 0x%x\r\n" COLOR_RESET, app_desc_ota.version, app_desc_ota.project_name, app_desc_ota.date, app_desc_ota.time, update_partition->subtype, (unsigned int)update_partition->address);
		...
		if (esp_https_ota_is_complete_data_received(https_ota_handle) != true)
		{
			...
			esp_https_ota_abort(https_ota_handle);
			cb->printf(COLOR_RESET COLOR_RED "Data was not received completely.\r\n" COLOR_RESET);
		...
		if ((err != ESP_OK) || (ota_finish_err != ESP_OK))
		{
			...
			{
				cb->printf(COLOR_RESET COLOR_RED "Image validation failed, image is corrupted.\r\n" COLOR_RESET);
				ESP_LOGE(TAG, "Image validation failed, image is corrupted.");
			}
			cb->printf(COLOR_RESET COLOR_RED "ESP_HTTPS_OTA upgrade failed 0x%x\r\n" COLOR_RESET, ota_finish_err);
			ESP_LOGE(TAG, "ESP_HTTPS_OTA upgrade failed 0x%x", ota_finish_err);
			exitcode = -10;
			goto exit;
		}
		cb->printf(COLOR_RESET COLOR_GREEN "ESP_HTTPS_OTA upgrade successful. Rebooting ...\r\n");
...
public:
	...
	int Execute(ShellCallback *cb, int argc, char *argv[]) override
	{
		...
		if (help->count > 0)
		{
			cb->printf(COLOR_RESET "Usage: %s\r\n", GetName());
			arg_print_syntax(fp, argtable, "\r\n");
			cb->printf("Do OTA processing\r\n");
			arg_print_glossary(fp, argtable, "  %-25s %s\r\n");
			cb->printf(COLOR_RESET);
			exitcode = 0;
			goto exit;
		}
		if (nerrors > 0)
		{
			...
			cb->printf(COLOR_RESET COLOR_RED);
			arg_print_errors(fp, end_arg, GetName());
			cb->printf("\r\n");
			cb->printf("'%s --help' for more information.\r\n COLOR_RESET", GetName());
			exitcode = 1;
			goto exit;
		}
		if (info->count > 0)
