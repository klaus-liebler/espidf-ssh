...
namespace sshd{
	...
	{
		ESP_LOGI(TAG, "channel_open!");
		SshDemon *sshd = static_cast<SshDemon*>(userdata);
		ConnectionCtx* cc = sshd->lookup_connectioncontext(session);
		if (cc == nullptr){
			ESP_LOGI(TAG, "cc == nullptr in " __FILE__);
			return nullptr;
		}
		if (cc->cc_channel!=nullptr){
			ESP_LOGI(TAG, "cc->cc_channel!=nullptr in " __FILE__);
			return nullptr;
		}
		ESP_LOGI(TAG, "create a new channel and store in ConnectionCtx and register callbacks (already prepared in demon!");	
	...
	void SshDemon::incoming_connection(ssh_bind sshbind, void *userdata)
	{
		...
		ESP_LOGI(TAG, "Set various session stuff");
		...
		ESP_LOGI(TAG, "Handle kex");
		if (ssh_handle_key_exchange(cc->cc_session) == SSH_ERROR)
		{
			ESP_LOGI(TAG, "Kex produced an error");
		 ...
		 */
		ESP_LOGI(TAG, "Add the newly generated session to event handling");
